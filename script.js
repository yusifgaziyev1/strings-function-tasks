...
function changeCharacter() {
    users.forEach((item) => {
        ...
        let continuationOfPhoneNumber = phoneSplit[1];
        let smashTheNumber = continuationOfPhoneNumber.split("").length - 2;
        let changeCharacter = continuationOfPhoneNumber.slice(0,smashTheNumber).split("");
        let lastNumber = phoneSplit[1].slice(smashTheNumber);
        let newData = changeCharacter.map((item) => item >= 0 ? "*" : item).join("");
        let combineWithEachOther = countryCode + newData + lastNumber;
        console.log(combineWithEachOther);
    })
}
...
function getPhoneUser (nickname) {
    const user = users.find((item) => nickname == item.username);
    if (user) {
...
function getPhoneUser2 (nickname) {
    const user = users.find((item) => nickname == item.username);
    if (user) {
        alert(user.phone)
    } else {
        alert("Belə bir istifadəçi yoxdur.")
    }
}

let getPromptUsername = prompt("Username daxil edin");
getPhoneUser2(getPromptUsername);

// 3) dogum ili 1990 ci ilden yuxari olanlari secen function yaradin

function getChangeYear() {
    const birthDate = users.map((item) => item.birthDate.split("-")[0] > 1990 ? console.log(item.username) : null);
}
...
function getChangeEmail () {
    const changeEmail = users.map((item) => item.email.replaceAll("x.dummyjson.com", "gmail.com"))
...
function getEyeColor(color) {
    const getEyePerson = users.map((item) => item.eyeColor == color ? console.log(item.username) : null);
}
